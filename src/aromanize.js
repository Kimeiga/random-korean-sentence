    ...
    for (var i = 0; i <= args.text.length; i++) {
      ...
      if (!isNaN(nextIdx) && nextIdx >= 0 && nextIdx <= 11171) {
        next =
          ...
          (nextIdx % 28 == 0
            ? ""
            : String.fromCharCode((nextIdx % 28) + 0x11a7)); // Index 0 is reserved for nothing
      } else {
      ...
      if (curr != null) {
        ...
        if (curr.length > 2) {
          ...
          if (
            rulemap.jong[curr.charAt(2) + next.charAt(0) + next.charAt(1)] !=
            undefined
          ) {
            res +=
              rulemap.jong[curr.charAt(2) + next.charAt(0) + next.charAt(1)];
...
//     /\?(.+&)?base(=true)?(&.+)?$/.test(document.currentScript.src) == false)
// ) {
//   // romanize()
